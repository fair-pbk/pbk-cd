...
PBK1 <- rxode2({ 
  ...
  GI_input = 
    fabs_ing * king_release * DIET_ing * wbw_f + fabs_ing * king_release * SOIL_ing + fabs_ing * king_release * COSM_ing + fabs_ing * king_release * DUST_ing +
    fabs_inh * kinh_release * k1_cig * CIG_inhal_ing + fabs_inh * kinh_release * k1_dust * AIR_inhal_ing * VInhalation + k4 * LUNG;
  
  d/dt(GUT) = GI_input - GUT; # GI-tract
  ...
  d/dt(FECES) = (1-k5)*GUT + k11*PLASMA + k15*LIVER ;                                        # cumulative output feces
  # d/dt(FECES) = (1-k5)*GUT + k11*PLASMA + k15*LIVER - FECES;                                        # daily output feces
  
  d/dt(URINE) = META * k17b + KIDNEY * k19x;                                                        # urine cumulative
  # d/dt(URINE) = META * k17b + KIDNEY * k19x - URINE;                                              # daily urine excretion
  ...
  d/dt(EXH) =
    (1 - k1_dust - k2_dust) * fabs_inh * kinh_release * AIR_inhal_ing * VInhalation + (1 - k1_cig  - k2_cig)  * fabs_inh * kinh_release * CIG_inhal_ing;          # cumulative output Exhalaltion
  
 # d/dt(EXH) =
 #  (1 - k1_dust - k2_dust) * fabs_inh * kinh_release * AIR_inhal_ing * VInhalation + (1 - k1_cig  - k2_cig)  * fabs_inh * kinh_release * CIG_inhal_ing - EXH;    # DAILY output Exhalaltion
  
  total = GUT + INTESTINE + LUNG + RBC + META + LIVER + KIDNEY + OTHER + PLASMA + UPTAKE1;           # Total Body burden
