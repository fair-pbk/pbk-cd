...
# Scenario: single adult male, age 30, default parameters
sex_i <- 1          # Male
...
# Single dose of 1 mg into GUT at age 30 (time 0 of the scenario)
ev_init <- merge(
  data.frame(id = 1, time = 30*365 - 1, evid = 1, cmt = "GUT", amt = 1000),
  phys_cov,
  by = c("id", "time")
)

# Event table: every simulation time is an observation row with its physiology
# covariates; the dose adds a second row at time 0 of the scenario.
ev <- rbind(
