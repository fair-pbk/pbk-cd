# TITLE ----
# R script to test the cadmium PBK.
#-------------------------------------------------------------------------------
# CREATION ----
# 07/04/26
#-------------------------------------------------------------------------------
...
# DESCRIPTION ----
# R script to test the cadmium PBK.
# This is the TG2025 model with a few additions or modifications, 
# marked ‘NEW ITEM’. It is not the model used at ANSES, which also includes 
# source-to-route tracking, but these two models provide the same results 
# if the sources are launched independently.

#-------------------------------------------------------------------------------
# REFERENCES ----
# [1] Gastellu, T., 2024. Cadmium_expo.R. https://zenodo.org/records/12772580.
# Consulté le 10/03/25.

# [2] Gastellu, T., Le Bizec, B., & Rivière, G. (2025). Integrating the lifelong 
# exposure dimension of a chemical mixture into the risk assessment process. 
# Application to trace elements. Food and Chemical Toxicology, 195, 115111.
# https://doi.org/10.1016/j.fct.2024.115111. p.3.
...
# Scenario 1: simulation of 24 hours for single adult age 30 with default 
# (fixed/constant – no variability) parameters and single dose of 1 mg at time 0
# >> 1mg / kg bw
...
filename1 <- paste0("Output/Output_test_pbk_cd/params_all_s1.qs")
qsave(params_all,file=filename1)


# PBK input 2: Influx event table

age_end <- 30*365+1 # 30yo + 1 day.
...
# Observed times
time_val <- c(30*365,30*365+1:24/365)-1
...
filename3 <- paste0("Output/Output_test_pbk_cd/event_res_s1.qs")
qsave(event_res,file=filename3)

# Solve
solve.pbk <- rxSolve(object=PBK1,
                     params=params_all,
                     events=event_res) %>%
  dplyr::filter(time %in% time_val) %>%
  dplyr::mutate(time=(time_val - (30*365-1))*365) %>%
  dplyr::select(time,ucdcr)


write.csv2(solve.pbk, file="Output/Output_test_pbk_cd/sim_s1_080426.csv",
           row.names=FALSE,fileEncoding="latin1")



res_graph <- ggplot() +
  geom_line(data = solve.pbk, 
            aes(x = time, 
                y = ucdcr)) +
  
   labs(
    x = "Time (h)",
    y = expression(bold(paste("Cadmiuria (µg Cd.(g creat."^-1,"))")))) +
  theme_minimal() +
  theme(
...
output_file <-"Output_test_pbk_cd/"
name_file <- paste0("Output/",output_file,"sim_s1_080426",
                    ".png")
...
filename1 <- paste0("Output/Output_test_pbk_cd/params_all_s2.qs")
qsave(params_all,file=filename1)
...
filename3 <- paste0("Output/Output_test_pbk_cd/event_res_s2.qs")
qsave(event_res,file=filename3)
...
write.csv2(solve.pbk, file="Output/Output_test_pbk_cd/sim_s2_080426.csv",
           row.names=FALSE,fileEncoding="latin1")
...
output_file <-"Output_test_pbk_cd/"
name_file <- paste0("Output/",output_file,"sim_s2_080426",
                    ".png")
